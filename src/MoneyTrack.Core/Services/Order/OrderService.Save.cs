...
namespace MoneyTrack.Core.Services
{
    ...
    {
        public async Task<ServiceModelResult<SaveResponse>> SaveAsync(int id, OrderRequest model)
        {
            var result = new ServiceModelResult<SaveResponse>();
            result.Model = new SaveResponse() { TypeName = nameof(Domain.Models.Order) };

            if (model == null)
                return BadRequest(result);

            var entities = await _context.Order.Where(x => x.Id == id).ToListAsync();
