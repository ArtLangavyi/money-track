...
namespace MoneyTrack.Core.Services
{
    ...
    {
        public async Task<ServiceModelResult<SaveResponse>> SaveAsync(int id, UserRequest model)
        {
            var result = new ServiceModelResult<SaveResponse>();
            result.Model = new SaveResponse() { TypeName = nameof(Domain.Models.User) };

            if (model == null)
                return BadRequest(result);

            var entities = await _context.Users.Where(x => x.Id == id).ToListAsync();
